...
inline real_t WENOCU6M2_P(real_t *f, real_t delta)
{
	...
	real_t c_q = 1000.0;																					   // C on page 7242
																											   // 	real_t q = 4.0;
	real_t a0 = 0.05 * sycl::pow((c_q + tau_6 / s1 * (beta_ave + chidelta2) / (s1 - epsdelta2 + chidelta2)), 4.0);   // alpha_0
	real_t a1 = 0.45 * sycl::pow((c_q + tau_6 / s2 * (beta_ave + chidelta2) / (s2 - epsdelta2 + chidelta2)), 4.0);   // alpha_1
	real_t a2 = 0.45 * sycl::pow((c_q + tau_6 / s3 * (beta_ave + chidelta2) / (s3 - epsdelta2 + chidelta2)), 4.0);   // alpha_2
	real_t a3 = 0.05 * sycl::pow((c_q + tau_6 / s64 * (beta_ave + chidelta2) / (s64 - epsdelta2 + chidelta2)), 4.0); // alpha_3
