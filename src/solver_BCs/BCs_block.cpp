...
// =======================================================
...
#ifdef USE_MPI
    MpiTrans* pTrans = setup.mpiTrans;

    #ifdef EXPLICIT_ALLOC
        auto &d_data = pTrans->d_mpiData;
        #define ACCESS_DATA(member) d_data.member
    #else
        auto *d_data = pTrans->d_mpiData;
        #define ACCESS_DATA(member) d_data->member
    #endif // end EXPLICIT_ALLOC

    real_t *send_xmin = ACCESS_DATA(TransBufSend_xmin);
    real_t *send_xmax = ACCESS_DATA(TransBufSend_xmax);
    real_t *recv_xmin = ACCESS_DATA(TransBufRecv_xmin);
    real_t *recv_xmax = ACCESS_DATA(TransBufRecv_xmax);

    real_t *send_ymin = ACCESS_DATA(TransBufSend_ymin);
    real_t *send_ymax = ACCESS_DATA(TransBufSend_ymax);
    real_t *recv_ymin = ACCESS_DATA(TransBufRecv_ymin);
    real_t *recv_ymax = ACCESS_DATA(TransBufRecv_ymax);

    real_t *send_zmin = ACCESS_DATA(TransBufSend_zmin);
    real_t *send_zmax = ACCESS_DATA(TransBufSend_zmax);
    real_t *recv_zmin = ACCESS_DATA(TransBufRecv_zmin);
    real_t *recv_zmax = ACCESS_DATA(TransBufRecv_zmax);
    #undef ACCESS_DATA

    BConditions bc_xmin = pTrans->neighborsBC[XMIN];
    BConditions bc_xmax = pTrans->neighborsBC[XMAX];
    BConditions bc_ymin = pTrans->neighborsBC[YMIN];
    BConditions bc_ymax = pTrans->neighborsBC[YMAX];
    BConditions bc_zmin = pTrans->neighborsBC[ZMIN];
    BConditions bc_zmax = pTrans->neighborsBC[ZMAX];
#endif // end USE_MPI
    ...
    q.submit([&](sycl::handler &h){
        h.parallel_for(sycl::nd_range<3>(global_ndrange_x, local_ndrange_x), [=](sycl::nd_item<3> index){
            ...
            FluidMpiCopyKernelX(i0, j, k, bl, send_xmin, d_UI, 0, -bl.Bwidth_X, BorToBuf);					// X_MIN
            FluidMpiCopyKernelX(i1, j, k, bl, send_xmax, d_UI, bl.Xmax - bl.Bwidth_X, bl.Bwidth_X, BorToBuf); // X_MAX
    });
    }).wait();

	std::chrono::high_resolution_clock::time_point start_time_x = std::chrono::high_resolution_clock::now();

    pTrans->MpiTransBuf(q, XDIR);
...
#if USE_MPI
                if (bc_xmin == BC_COPY) 
                    FluidMpiCopyKernelX(i0, j, k, bl, recv_xmin, d_UI, 0, -bl.Bwidth_X, BufToBC);
                else
#endif // USE_MPI
                    FluidBCKernelX(i0, j, k, bl, BC0, d_UI, 0, bl.Bwidth_X, 1);
#ifdef USE_MPI
                if (bc_xmax == BC_COPY)
                    FluidMpiCopyKernelX(i1, j, k, bl, recv_xmax, d_UI, bl.Xmax - bl.Bwidth_X, bl.Bwidth_X, BufToBC);
                else
#endif // USE_MPI
                    FluidBCKernelX(i1, j, k, bl, BC1, d_UI, bl.X_inner, bl.Xmax - bl.Bwidth_X - 1, -1); 
        });
	...
	q.submit([&](sycl::handler &h){
        h.parallel_for(sycl::nd_range<3>(global_ndrange_y, local_ndrange_y), [=](sycl::nd_item<3> index){
            ...
            FluidMpiCopyKernelY(i, j0, k, bl, send_ymin, d_UI, 0, -bl.Bwidth_Y, BorToBuf);					// X_MIN
            FluidMpiCopyKernelY(i, j1, k, bl, send_ymax, d_UI, bl.Ymax - bl.Bwidth_Y, bl.Bwidth_Y, BorToBuf); // X_MAX
    });}).wait();

	std::chrono::high_resolution_clock::time_point start_time_y = std::chrono::high_resolution_clock::now();

    pTrans->MpiTransBuf(q, YDIR);
...
#if USE_MPI
            if (bc_ymin == BC_COPY)
                FluidMpiCopyKernelY(i, j0, k, bl, recv_ymin, d_UI, 0, -bl.Bwidth_Y, BufToBC);
            else
#endif // USE_MPI
                FluidBCKernelY(i, j0, k, bl, BC2, d_UI, 0, bl.Bwidth_Y, 1);
#ifdef USE_MPI
            if (bc_ymax == BC_COPY)
                FluidMpiCopyKernelY(i, j1, k, bl, recv_ymax, d_UI, bl.Ymax - bl.Bwidth_Y, bl.Bwidth_Y, BufToBC);
            else
#endif // USE_MPI
                FluidBCKernelY(i, j1, k, bl, BC3, d_UI, bl.Y_inner, bl.Ymax - bl.Bwidth_Y - 1, -1); //
    }); }).wait();
	...
	q.submit([&](sycl::handler &h){
        h.parallel_for(sycl::nd_range<3>(global_ndrange_z, local_ndrange_z), [=](sycl::nd_item<3> index){
            ...
            FluidMpiCopyKernelZ(i, j, k0, bl, send_zmin, d_UI, 0, -bl.Bwidth_Z, BorToBuf);					 // X_MIN
            FluidMpiCopyKernelZ(i, j, k1, bl, send_zmax, d_UI, bl.Zmax - bl.Bwidth_Z, bl.Bwidth_Z, BorToBuf); // X_MAX
    }); }).wait();

	std::chrono::high_resolution_clock::time_point start_time_z = std::chrono::high_resolution_clock::now();
    pTrans->MpiTransBuf(q, ZDIR);
	std::chrono::high_resolution_clock::time_point end_time_z = std::chrono::high_resolution_clock::now();
	duration_z = std::chrono::duration<float, std::milli>(end_time_z - start_time_z).count();
...
#if USE_MPI
            if (bc_zmin == BC_COPY) 
                FluidMpiCopyKernelZ(i, j, k0, bl, recv_zmin, d_UI, 0, -bl.Bwidth_Z, BufToBC);
            else
#endif // USE_MPI
                FluidBCKernelZ(i, j, k0, bl, BC4, d_UI, 0, bl.Bwidth_Z, 1);
#ifdef USE_MPI
            if (bc_zmax == BC_COPY)
                FluidMpiCopyKernelZ(i, j, k1, bl, recv_zmax, d_UI, bl.Zmax - bl.Bwidth_Z, bl.Bwidth_Z, BufToBC);
            else
#endif
            FluidBCKernelZ(i, j, k1, bl, BC5, d_UI, bl.Z_inner, bl.Zmax - bl.Bwidth_Z - 1, -1); //
    }); }).wait();
...
// =======================================================
	...
	if (bl.DimX)
	{
		auto local_ndrange_x = sycl::range<3>(bl.dim_block_z, bl.dim_block_y, bl.Bwidth_X); // size of workgroup
		auto global_ndrange_x = sycl::range<3>((bl.Zmax + local_ndrange_x[0] - 1) / local_ndrange_x[0] * local_ndrange_x[0],
											   (bl.Ymax + local_ndrange_x[1] - 1) / local_ndrange_x[1] * local_ndrange_x[1],
											   bl.Bwidth_X);
	...
	q.submit([&](sycl::handler &h)
			 { h.parallel_for(sycl::nd_range<3>(global_ndrange_x, local_ndrange_x), [=](sycl::nd_item<3> index)
				[[sycl::reqd_sub_group_size(32)]]
                {
								  ...
								  int j = index.get_global_id(1);
								  int i0 = index.get_global_id(2) + 0;
								  int i1 = index.get_global_id(2) + bl.Xmax - bl.Bwidth_X;

                                  if (k < bl.Zmax && j < bl.Ymax) {
								      FluidMpiCopyKernelX(i0, j, k, bl, ptr_TransBufSend_xmin, d_UI, 0, -bl.Bwidth_X, BorToBuf);					// X_MIN
								      FluidMpiCopyKernelX(i1, j, k, bl, ptr_TransBufSend_xmax, d_UI, bl.Xmax - bl.Bwidth_X, bl.Bwidth_X, BorToBuf); // X_MAX
                                  }
							  }); })
	...
	if (bl.DimY)
	{
		auto local_ndrange_y = sycl::range<3>(bl.dim_block_z, bl.Bwidth_Y, bl.dim_block_x); // size of workgroup
		auto global_ndrange_y = sycl::range<3>((bl.Zmax + local_ndrange_y[0] - 1) / local_ndrange_y[0] * local_ndrange_y[0],
											   bl.Bwidth_Y,
											   (bl.Xmax + local_ndrange_y[2] - 1) / local_ndrange_y[2] * local_ndrange_y[2]);
	...
	if (bl.DimZ)
	{
		auto local_ndrange_z = sycl::range<3>(bl.Bwidth_Z, bl.dim_block_y, bl.dim_block_x); // size of workgroup
		auto global_ndrange_z = sycl::range<3>(bl.Bwidth_Z,
											   (bl.Ymax + local_ndrange_z[1] - 1) / local_ndrange_z[1] * local_ndrange_z[1],
											   (bl.Xmax + local_ndrange_z[2] - 1) / local_ndrange_z[2] * local_ndrange_z[2]);
	...
	q.submit([&](sycl::handler &h)
			 { h.parallel_for(
				   ...
				   [[sycl::reqd_sub_group_size(32)]]
                   {
					   int k0 = index.get_global_id(0) + 0;
					   int k1 = index.get_global_id(0) + bl.Zmax - bl.Bwidth_Z;
					   int j = index.get_global_id(1);
					   int i = index.get_global_id(2);

                       if (j < bl.Ymax && i < bl.Xmax) {
					       FluidMpiCopyKernelZ(i, j, k0, bl, ptr_TransBufSend_zmin, d_UI, 0, -bl.Bwidth_Z, BorToBuf);					 // X_MIN
					       FluidMpiCopyKernelZ(i, j, k1, bl, ptr_TransBufSend_zmax, d_UI, bl.Zmax - bl.Bwidth_Z, bl.Bwidth_Z, BorToBuf); // X_MAX
                       }
				   }); })
...
// std::cout << "[DEBUG] Boundary Conditions pass" << std::endl;
