...
void SYCLSolver::Output_vti(int rank, std::ostringstream &timeFormat, std::ostringstream &stepFormat, std::ostringstream &rankFormat, bool error)
{
	...
	if (error)
	{
		...
		{
			variables_names[index] = "E-Dim[" + std::to_string(mm) + "]";
			index++;
			variables_names[index] = "E-Dimtemp1[" + std::to_string(mm) + "]";
			index++;
			variables_names[index] = "E-Dimtemp2[" + std::to_string(mm) + "]";
			index++;
		}
#endif
#endif
#if DIM_X
		Onbvar += (3 + NUM_COP + Emax * 2);
